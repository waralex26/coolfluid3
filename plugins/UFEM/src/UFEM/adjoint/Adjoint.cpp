    ...
    // CFinfo << "Test assembly" << CFendl;
      auto region_action = create_proto_action(region->name(), elements_expression(boost::mpl::vector2<      mesh::LagrangeP1::Line2D,
          mesh::LagrangeP1::Triag3D>(), group(
                            ...
                            element_quadrature
                            (
                              _A(U[_i], U[_i]) += transpose(N(U))*N(U)*u[_i]* lit(4) * lit(m_a[Nt])/(lit(1)-lit(m_a[Nt]))/ lit(m_th)*density_ratio * normal[_i]/_norm(normal),
                              _a[U[_i]] += transpose(N(U)) * lit(6.0) * u[0] * u[0] * lit(m_a[Nt])/(1 - lit(m_a[Nt])) * normal[_i]/_norm(normal) * density_ratio
