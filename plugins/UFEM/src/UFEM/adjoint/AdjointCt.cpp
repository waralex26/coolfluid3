      ...
      (
        _A = _0, _T = _0, _a = _0,
          ...
          (
                  ...
                  _a[U[_i]] +=  transpose(N(U) -tau_su*u*nabla(U)) * lit(3.0) / lit(2.0) * lit(0.7) * uDisk[_i] * lit(12.9952) / lit(m_th) +  /* -transpose(N(U) - tau_su*u*nabla(U)) * lit(3.0) * g[_i] * density_ratio + */
    ...
    // CFinfo << "Test assembly" << CFendl;
      auto region_action = create_proto_action(region->name(), elements_expression(boost::mpl::vector2<mesh::LagrangeP1::Line2D,
          mesh::LagrangeP1::Triag3D>(), group(
                            ...
                            element_quadrature
                            (
                                _A(U[_i], U[_i]) += transpose(N(U)) * N(U) * lit(0.7) * lit(12.9952)  /* / lit(m_th) */  * normal[_i]
