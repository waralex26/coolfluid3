      ...
      (
        _A = _0, _T = _0, _a = _0,
          ...
          (
                  ...
                  _a[U[_i]] += transpose(N(U) -tau_su*u*nabla(U)) * lit(3.0) / lit(2.0) * Ct * uDisk * uDisk / lit(m_th) * normal[_i]/_norm(normal) + /* -transpose(N(U) - tau_su*u*nabla(U)) * 3 * g[_i] * density_ratio + */
    ...
    // CFinfo << "Test assembly" << CFendl;
      auto region_action = create_proto_action(region->name(), elements_expression(boost::mpl::vector2<mesh::LagrangeP1::Line2D,
          mesh::LagrangeP1::Triag3D>(), group(
                            ...
                            element_quadrature
                            (
                                _A(U[_i], U[_i]) += transpose(N(U)) * N(U) * Ct * uDisk  /* / lit(m_th) */  * normal[_i]
                                // _a[U[_i]] += transpose(N(U)) * lit(3.0) / lit(2.0) * Ct * uDisk * uDisk * normal[_i]
                            ), // integrate
